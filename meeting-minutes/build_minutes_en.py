...
FONT_DIR = Path("/root/.claude/skills/synced/2647aceb-a845-4b12-90dc-91d8ab71d9a6_7433ad11-26c2-4379-bdad-8cc91f648c3b/meeting-minutes/assets/fonts")
...
def header_block(d):
    ...
    out = [Mono(kicker, size=8.2, color=GREEN), Spacer(1, 12),
           P(L(d["title"], "en"), ST["title"]), Spacer(1, 20)]

    cells = []
    for key, label in (("date", "Date"), ("meeting", "Meeting"),
                       ("chair", "Chair"), ("present", "Present")):
        v = d.get(key, {})
...
def atomic_columns(sec):
    items = [P(L(sec.get("headline"), "en"), ST["headline"]), Spacer(1, 9)]
...
def _atomic_columns_bilingual(sec):
...
def split_table(n, sec):
    ...
    rows = [[header], [P(L(sec.get("headline"), "en"), ST["headline"])]]
...
def action_table(actions):
    ...
    for a in actions:
        ...
        rows.append([
            ...
            P(L(a.get("action"), "en") if False else "", ST["act_en"]) if False else None,
            P(L(a.get("owner"), "en"), ST["owner_en"]),
            due_cell,
        ])
    widths = [27.2, CONTENT_W - 27.2 - 110 - 120, 110, 120]
...
def build(d, out):
    ...
    FooterCanvas.footer_left = d.get("footer", f"{d.get('org', 'Dar Al-’Ulum Montréal')} · Committee Meeting Minutes")
    ...
    doc = BaseDocTemplate(str(out), pagesize=letter,
                          ...
                          title=f"Committee Meeting Minutes, {date_short}",
